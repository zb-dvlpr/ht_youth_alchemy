...
export type MobileFloatingActionMenuPosition = {
  x: number;
  y: number;
};

type MobileFloatingActionMenuProps = {
  toggleLabel: string;
  position: MobileFloatingActionMenuPosition;
  onPositionChange: (position: MobileFloatingActionMenuPosition) => void;
  children:
    | ReactNode
    | ((helpers: { closeMenu: () => void }) => ReactNode);
  className?: string;
...
export default function MobileFloatingActionMenu({
  toggleLabel,
  position,
  onPositionChange,
  children,
  ...
  const commitPosition = (nextPosition: MobileFloatingActionMenuPosition) => {
    const clamped =
      viewport.width > 0 && viewport.height > 0
  ...
  const startMomentum = (
    nextPosition: MobileFloatingActionMenuPosition,
  ...
  return (
    <>
      <button
        ref={buttonRef}
        type="button"
        className={`${styles.mobileYouthMenuButton}${className ? ` ${className}` : ""}`}
